...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        foreach ($this->getJobIds() as $id) {
            $job = $this->vdes['queue.failer']->find($id);

            if (is_null($job)) {
                $this->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->vdes['events']->dispatch(new JobRetryRequested($job));

                $this->retryJob($job);

                $this->info("The failed job [{$id}] has been pushed back onto the queue!");

                $this->vdes['queue.failer']->forget($id);
            }
        }
    }
    ...
    protected function getJobIds()
    {
        ...
        if (count($ids) === 1 && $ids[0] === 'all') {
            return Arr::pluck($this->vdes['queue.failer']->all(), 'id');
        }
    ...
    {
        $ids = collect($this->vdes['queue.failer']->all())
                        ->where('queue', $queue)
                        ->pluck('id')
                        ->toArray();
    ...
    {
        $this->vdes['queue']->connection($job->connection)->pushRaw(
    ...
    protected function refreshRetryUntil($payload)
    {
        ...
        } elseif ($this->vdes->bound(Encrypter::class)) {
            $instance = unserialize($this->vdes->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
