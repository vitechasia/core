...
class PendingRequest
{
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $vdesData = $this->parseRequestData($method, $url, $options);
    ...
    {
        $vdesData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = Str::of($url);

        if (empty($vdesData) && $method === 'GET' && $urlString->contains('?')) {
            $vdesData = (string) $urlString->after('?');
        }

        if (is_string($vdesData)) {
            parse_str($vdesData, $parsedData);

            $vdesData = is_array($parsedData) ? $parsedData : [];
        }
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    optional($this->factory)->recordRequestResponsePair(
                        (new Request($request))->withData($options['vdes_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? collect())
                     ->map
                     ->__invoke((new Request($request))->withData($options['vdes_data']), $options)
                     ->filter()
                     ->first();
        ...
        return tap($request, function (&$request) use ($options) {
            $this->beforeSendingCallbacks->each(function ($callback) use (&$request, $options) {
                $callbackResult = call_user_func(
                    $callback, (new Request($request))->withData($options['vdes_data']), $options, $this
                );
